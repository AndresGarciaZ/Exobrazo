...
esp_err_t move_handler(httpd_req_t *req) {
    ...
    if (json == NULL) {
         httpd_resp_send_400(req, "JSON invalido");
         return ESP_FAIL;
    }
    ...
    if (xQueueSend(webCommandQueue, &cmd, pdMS_TO_TICKS(100)) != pdTRUE) {
        httpd_resp_send_500(req, "Error procesando comando");
        return ESP_FAIL;
    }
...
    config.priority = 5; // Prioridad normal
...
                exobrazo.moverMotorWeb(
                    receivedCommand.motorIndex,
                    receivedCommand.aceleracion,
                    receivedCommand.direccion,
                    receivedCommand.pasos
                );
